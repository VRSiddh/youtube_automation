...
;// ./src/remotion/components/AudioTrack.tsx
...
const AudioTrack = ({
  ...
  const voiceDurationFrames = narrationEndFrame - introPauseFrames;
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: introPauseFrames, durationInFrames: voiceDurationFrames, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Audio, { src: (0,esm.staticFile)(voiceRelativePath) }) }),
    hasBed && /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: 0, durationInFrames: narrationEndFrame, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Audio, { src: (0,esm.staticFile)(bedRelativePath), volume: 0.1 }) })
  ] });
};

;// ./src/remotion/components/BackgroundVideo.tsx
...
const BackgroundVideo = ({ src }) => {
  ...
  const scale = (0,esm.interpolate)(frame, [0, durationInFrames], [1, 1.1], {
  ...
  if (!src) {
    return /* @__PURE__ */ (0,jsx_runtime.jsx)(
      esm.AbsoluteFill,
      {
        style: {
          overflow: "hidden",
          background: "radial-gradient(circle at 30% 20%, #1a1a2e 0%, #0f0f1a 45%, #050508 100%)",
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { style: { overflow: "hidden", backgroundColor: "#0a0a0a" }, children: [
    ...
    /* @__PURE__ */ (0,jsx_runtime.jsx)(
      ...
      {
        style: {
          background: "linear-gradient(to bottom, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0.55) 45%, rgba(0,0,0,0.7) 100%)",
          pointerEvents: "none"
        }
      }
    )
...
;// ./src/remotion/utils/chunkWords.ts

function chunkWords(timestamps) {
  if (timestamps.length === 0) return [];
  const chunks = [];
  let i = 0;
  while (i < timestamps.length) {
    const remaining = timestamps.length - i;
    if (remaining === 1) {
      const orphan = timestamps[i];
      if (chunks.length > 0) {
        const last = chunks[chunks.length - 1];
        last.words.push(orphan);
        last.endMs = orphan.endMs;
      } else {
        chunks.push({
          words: [orphan],
    ...
    const slice = timestamps.slice(i, i + take);
    chunks.push({
      words: slice,
...
;// ./src/remotion/components/KineticCaptions.tsx





function findActiveChunk(chunks, tMs) {
  if (tMs < 0) return null;
  for (let i = 0; i < chunks.length; i++) {
    const c = chunks[i];
    if (tMs >= c.startMs && tMs <= c.endMs + 24) {
      return { chunk: c, chunkIndex: i };
    }
  }
  return null;
}
const STROKE = "2px 2px 0 #000, -2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000, 0 4px 12px rgba(0,0,0,0.9)";
const KineticCaptions = ({
  wordTimestamps,
  themeSeed,
  introPauseFrames,
  narrationEndFrame
  ...
  const chunks = (0,react.useMemo)(() => chunkWords(wordTimestamps), [wordTimestamps]);
  const audioTimeMs = frame < introPauseFrames ? -1 : (frame - introPauseFrames) / fps * 1e3;
  const active = findActiveChunk(chunks, audioTimeMs);
  if (frame >= narrationEndFrame || !active || audioTimeMs < 0) {
    return null;
  }
  const { chunk, chunkIndex } = active;
  const chunkStartFrame = introPauseFrames + Math.max(0, Math.floor(chunk.startMs / 1e3 * fps));
  const subFrame = frame - chunkStartFrame;
  const rowPop = (0,esm.spring)({
    frame: subFrame,
    fps,
    config: { damping: 16, mass: 0.55, stiffness: 200 }
  });
  const rowScale = 0.92 + rowPop * 0.08;
  const n = chunk.words.length;
  const fontSize = n >= 3 ? 70 : 88;
  const gapPx = n >= 3 ? 22 : 30;
  const highlight = themeSeed % 2 === 0 ? "#fff566" : "#7fff7f";
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(
    "div",
    {
      style: {
      ...
      children: /* @__PURE__ */ (0,jsx_runtime.jsx)(
        "div",
        {
          ...
          children: chunk.words.map((w, wi) => {
            const isActive = audioTimeMs >= w.startMs && audioTimeMs < w.endMs;
            return /* @__PURE__ */ (0,jsx_runtime.jsx)(
              "span",
              {
                style: {
...
const ProgressBar = ({
  ...
  const frame = (0,esm.useCurrentFrame)();
  const { durationInFrames } = (0,esm.useVideoConfig)();
  const pct = durationInFrames > 0 ? frame / durationInFrames * 100 : 0;
  const inPoll = frame >= narrationEndFrame && frame < narrationEndFrame + 150;
  const opacity = inPoll ? (0,esm.interpolate)(
    frame,
    [narrationEndFrame, narrationEndFrame + 20],
    [0.5, 1],
    { extrapolateRight: "clamp" }
  ) : 0.45;
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(
...
;// ./src/remotion/components/RedditHeader.tsx



const hashHue = (seed) => Math.abs(seed * 7919 % 360);
function formatRedditCount(n) {
  if (n >= 1e6) return `${(n / 1e6).toFixed(1).replace(/\.0$/, "")}m`;
  if (n >= 1e3) return `${(n / 1e3).toFixed(1).replace(/\.0$/, "")}k`;
  return String(n);
}
function pseudoTimeLabel(seed) {
  const labels = ["2h", "5h", "1d", "2d", "3d"];
...
const RedditHeader = ({
  ...
  const frame = (0,esm.useCurrentFrame)();
  const start = Math.round(ups * 0.4);
  const displayUps = Math.round(
    (0,esm.interpolate)(frame, [0, 90], [start, ups], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: (t) => 1 - (1 - t) * (1 - t)
    })
  );
  const shortTitle = title.length > 140 ? `${title.slice(0, 138).trim()}\u2026` : title;
  const hue = hashHue(themeSeed);
  const user = author && author !== "[deleted]" ? author : "throwaway";
  const timeAgo = pseudoTimeLabel(themeSeed);
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(
    "div",
    {
      style: {
                        ...
                        /* @__PURE__ */ (0,jsx_runtime.jsx)(
                          "div",
                          {
                            style: {
                              ...
                              borderRadius: "50%",
                              flexShrink: 0,
                              background: `linear-gradient(145deg, hsl(${hue},72%,48%), hsl(${(hue + 55) % 360},58%,32%))`,
                              boxShadow: "inset 0 2px 6px rgba(255,255,255,0.2), 0 4px 14px rgba(0,0,0,0.5)",
                              border: "2px solid #2a2a2b"
                            }
                          }
                        ),
                        /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { flex: 1, minWidth: 0, paddingTop: 2 }, children: [
                          /* @__PURE__ */ (0,jsx_runtime.jsxs)(
                            "div",
                            {
                              style: {
                          ...
                          /* @__PURE__ */ (0,jsx_runtime.jsxs)(
                            "div",
                            {
                              ...
                              children: [
                                /* @__PURE__ */ (0,jsx_runtime.jsxs)("span", { style: { color: "#c8cbcd" }, children: [
                                  "u/",
                                  user
                                ] }),
                                /* @__PURE__ */ (0,jsx_runtime.jsx)("span", { style: { margin: "0 8px", opacity: 0.6 }, children: "\xB7" }),
                                /* @__PURE__ */ (0,jsx_runtime.jsx)("span", { children: timeAgo })
                              ]
                            }
                          )
                        ] })
                            ...
                            children: [
                              ...
                              /* @__PURE__ */ (0,jsx_runtime.jsx)(
                                "span",
                                {
                                  ...
                                  children: formatRedditCount(displayUps)
                                }
                              ),
                              /* @__PURE__ */ (0,jsx_runtime.jsx)(
                                "span",
                            ...
                            children: [
                              ...
                              /* @__PURE__ */ (0,jsx_runtime.jsx)(
                                "span",
                                {
                                  ...
                                  children: formatRedditCount(numComments)
                                }
                              )
                            ]
                          }
...
;// ./src/remotion/components/SplitPoll.tsx



const SplitPoll = ({
  narrationEndFrame,
  tickAudioRelativePath,
  hasTickAudio
}) => {
  const frame = (0,esm.useCurrentFrame)();
  const { fps, height } = (0,esm.useVideoConfig)();
  const pollStart = narrationEndFrame;
  const pollEnd = narrationEndFrame + 150;
  if (frame < pollStart || frame >= pollEnd) {
    return null;
  }
  const local = frame - pollStart;
  ...
  const lineOpacity = Math.min(1, local / 25);
  const pollBandHeight = Math.round(height * 0.4);
  const titleSize = Math.round(pollBandHeight * 0.12);
  const line2Size = Math.round(pollBandHeight * 0.15);
  const lineGap = Math.round(pollBandHeight * 0.025);
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { style: { zIndex: 12, pointerEvents: "none" }, children: [
    hasTickAudio && [60, 90, 120].map((f) => /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: pollStart + f, durationInFrames: 8, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Audio, { src: (0,esm.staticFile)(tickAudioRelativePath), volume: 0.9 }) }, f)),
    /* @__PURE__ */ (0,jsx_runtime.jsxs)(
      "div",
      {
        style: {
          ...
          /* @__PURE__ */ (0,jsx_runtime.jsxs)(
            "div",
            {
              style: {
                ...
                alignItems: "center",
                justifyContent: "center",
                background: "linear-gradient(180deg, rgba(180,40,40,0.45) 0%, rgba(40,0,0,0.55) 100%)",
                ...
                /* @__PURE__ */ (0,jsx_runtime.jsx)(
                  "div",
                  {
                    style: {
                      ...
                      textShadow: "3px 3px 0 #000, -2px -2px 0 #000, 0 0 18px rgba(0,0,0,0.8)",
                      lineHeight: 1.05
                    },
                    children: "YOU ARE"
                  }
                ...
                /* @__PURE__ */ (0,jsx_runtime.jsx)(
                  "div",
                  {
                    style: {
                      ...
                      textAlign: "center",
                      letterSpacing: "-0.02em",
                      textShadow: "4px 4px 0 #000, -2px -2px 0 #000, 0 0 22px rgba(0,0,0,0.85)",
                      lineHeight: 1
                    },
                    children: "THE JERK"
                  }
          ...
          /* @__PURE__ */ (0,jsx_runtime.jsx)(
            "div",
            {
              style: {
                ...
                alignSelf: "stretch",
                flexShrink: 0,
                background: "linear-gradient(180deg, rgba(255,255,255,0.95) 0%, rgba(200,200,220,0.85) 100%)",
                opacity: lineOpacity,
                zIndex: 2,
          ...
          /* @__PURE__ */ (0,jsx_runtime.jsxs)(
            "div",
            {
              style: {
                ...
                alignItems: "center",
                justifyContent: "center",
                background: "linear-gradient(180deg, rgba(40,80,180,0.45) 0%, rgba(0,20,60,0.55) 100%)",
                border: "4px solid rgba(100,160,255,0.95)",
                borderRadius: 16,
                marginLeft: -2,
                boxShadow: "inset 0 0 40px rgba(0,0,0,0.35)"
              },
...
const AitaVideo = (props) => {
  ...
  const frame = (0,esm.useCurrentFrame)();
  const blackStart = durationInFrames - 30;
  const showBlack = frame >= blackStart;
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { style: { backgroundColor: "#000" }, children: [
    !showBlack && /* @__PURE__ */ (0,jsx_runtime.jsxs)(jsx_runtime.Fragment, { children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)(BackgroundVideo, { src: backgroundVideoSrc }),
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
        RedditHeader,
        {
          ...
          numComments: redditMeta.numComments,
          themeSeed
        }
      ),
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
      ...
      /* @__PURE__ */ (0,jsx_runtime.jsx)(ProgressBar, { narrationEndFrame }),
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
        SplitPoll,
        {
          narrationEndFrame,
          tickAudioRelativePath: tickAudioSrc,
          hasTickAudio: hasTick
        }
      )
    ] }),
    !skipAudio && /* @__PURE__ */ (0,jsx_runtime.jsx)(
      AudioTrack,
      {
        voiceRelativePath: voiceAudioSrc,
        bedRelativePath: bedAudioSrc,
        introPauseFrames,
        narrationEndFrame,
        hasBed
      }
    ),
    showBlack && /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.AbsoluteFill, { style: { backgroundColor: "#000", zIndex: 100 } })
  ] });
};

;// ./src/remotion/defaultProps.ts

function buildMockTimestamps(text) {
  const words = text.split(/\s+/).filter(Boolean);
  const msPerWord = 380;
  return words.map((word, i) => ({
    word,
    startMs: i * msPerWord,
    endMs: (i + 1) * msPerWord - 30
  }));
}
const mockScript = "This is a Remotion Studio preview. Add your API keys and run the pipeline to generate real stories. The captions sync to timestamps from ElevenLabs. So... Am I the jerk here?";
const mockWords = buildMockTimestamps(mockScript);
const lastEnd = mockWords[mockWords.length - 1].endMs;
const fps = 30;
const introPauseFrames = Math.ceil(750 / 1e3 * fps);
const voiceFrames = Math.max(1, Math.ceil(lastEnd / 1e3 * fps));
const narrationEndFrame = introPauseFrames + voiceFrames;
const durationInFrames = narrationEndFrame + 150 + 30;
const defaultAitaProps = {
  scriptText: mockScript,
  redditMeta: {
    subreddit: "AITAH",
    title: "Preview \u2014 run npm run pipeline with keys to generate a real video",
    ups: 12400,
    permalink: "https://www.reddit.com/r/AITAH/",
    author: "preview_user",
    numComments: 2100
  },
  wordTimestamps: mockWords,
  voiceAudioSrc: "audio/voice.mp3",
  backgroundVideoSrc: null,
  bedAudioSrc: "music/bed.mp3",
  tickAudioSrc: "sfx/tick.mp3",
  hasBed: false,
  hasTick: false,
  skipAudio: true,
  introPauseFrames,
...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "aita-video-factory:";
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkaita_video_factory"] = self["webpackChunkaita_video_factory"] || [];
